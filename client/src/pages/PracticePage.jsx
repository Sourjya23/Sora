...
export default function PracticePage() {
        ...
        if (language === "react") {
          html = `
            ...
            <head>
              ...
              <style>body { font-family: sans-serif; color: white; background: #0f172a; padding: 1rem; }</style>
            </head>
            <body>
              <div id="root"></div>
              <script type="text/babel">
                try {
                  ...
                  document.getElementById('root').innerHTML = '<div style="color:#fb7185; font-family:monospace;">' + err.message + '</div>';
                }
              </script>
            </body>
            </html>
          ...
          html = `<!DOCTYPE html><html><head><style>body{color:white;background:#0f172a;}</style></head><body>${editorCode}</body></html>`;
        }
  ...
  const renderText = (text) => {
    ...
    return parts.map((part, i) => {
      if (part.startsWith("```")) {
        ...
        return (
          <pre key={i} className="bg-slate-950 border border-slate-800 rounded-xl p-4 my-3 text-sm font-mono text-emerald-400 overflow-x-auto whitespace-pre-wrap">
            {code}
          </pre>
        );
      }
  ...
  return (
    <div className="h-screen w-full bg-slate-950 flex flex-col overflow-hidden font-sans text-slate-200">
      ...
      <header className="h-14 border-b border-slate-900 bg-slate-950 flex items-center justify-between px-6 z-10 shrink-0">
        <div className="flex items-center gap-4">
          ...
          {currentTopic && (
            <span className="text-[10px] bg-violet-500/10 border border-violet-500/20 text-violet-400 px-3 py-1 rounded-full font-bold uppercase tracking-wider">
              {currentTopic}
            </span>
          )}
        </div>
        ...
        <form onSubmit={handleGenerateLesson} className="flex items-center gap-2 max-w-lg flex-1 mx-8">
          <div className="flex-1 relative">
            <input
              ...
              className="w-full bg-slate-900 border border-slate-800 rounded-xl px-4 py-2.5 text-sm text-slate-200 outline-none focus:border-violet-500 placeholder:text-slate-600 transition-colors"
            />
          </div>
          <button
            type="submit"
            disabled={loadingLesson || !topicInput.trim()}
            className="bg-violet-600 hover:bg-violet-500 text-white text-sm font-bold px-5 py-2.5 rounded-xl disabled:opacity-50 disabled:cursor-not-allowed transition-all shadow-lg shadow-violet-600/20"
          >
            {loadingLesson ? (
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <div className="w-[500px] shrink-0 border-r border-slate-900 flex flex-col bg-[#0a0d14]">

          {/* Tabs */}
          <div className="flex bg-slate-900 border-b border-slate-800 shrink-0">
            <button
              onClick={() => setLeftTab("lesson")}
              className={`flex-1 py-2.5 text-xs font-bold uppercase tracking-wider transition-colors ${leftTab === "lesson" ? "bg-violet-600/10 text-violet-400 border-b-2 border-violet-500" : "text-slate-500 hover:bg-slate-800 hover:text-slate-300"}`}
            >
              📚 Lesson
            </button>
            <button
              onClick={() => setLeftTab("chat")}
              className={`flex-1 py-2.5 text-xs font-bold uppercase tracking-wider transition-colors ${leftTab === "chat" ? "bg-violet-600/10 text-violet-400 border-b-2 border-violet-500" : "text-slate-500 hover:bg-slate-800 hover:text-slate-300"}`}
            >
              💬 Ask AI
            </button>
          </div>
            ...
            <div className="flex-1 flex flex-col overflow-hidden">
              {!lesson && !loadingLesson ? (
                ...
                <div className="flex-1 flex flex-col items-center justify-center p-8 text-center">
                  ...
                  <p className="text-slate-500 text-sm max-w-xs">Type any programming topic above and I'll generate a complete, interactive lesson for you.</p>
                  <div className="mt-6 flex flex-wrap gap-2 justify-center">
                    {["React Hooks", "Binary Search", "SQL Joins", "REST APIs", "Python OOP"].map(s => (
                      <button
                        key={s}
                        onClick={() => { setTopicInput(s); }}
                        className="text-xs bg-slate-900 border border-slate-800 text-slate-400 px-3 py-1.5 rounded-lg hover:border-violet-500/30 hover:text-violet-400 transition-colors"
                      >
                        {s}
                      </button>
                    ))}
                  ...
                  {lessonError && (
                    <div className="mt-8 p-4 bg-rose-500/10 border border-rose-500/20 rounded-xl max-w-md">
                      ...
                      <p className="text-rose-400/80 text-xs mt-2 text-left">{lessonError}</p>
                    </div>
                  )}
                </div>
              ) : loadingLesson ? (
                ...
                <div className="flex-1 flex flex-col items-center justify-center p-8">
                  ...
                  <p className="text-violet-400 font-bold mt-6 text-sm">Generating your lesson...</p>
                ...
                <div className="flex-1 overflow-y-auto">
                  {/* Section Tabs */}
                  <div className="flex overflow-x-auto border-b border-slate-800 bg-slate-900/50 shrink-0 scrollbar-hide">
                    {lesson.sections.map((section, idx) => (
                      <button
                        key={idx}
                        onClick={() => handleSectionClick(idx)}
                        className={`px-4 py-3 text-xs font-bold whitespace-nowrap border-b-2 transition-colors ${activeSection === idx ? "border-violet-500 text-violet-400 bg-violet-500/10" : "border-transparent text-slate-500 hover:text-slate-300 hover:bg-slate-800"}`}
                      >
                  ...
                  {lesson.sections[activeSection] && (
                    <div className="p-5 space-y-5">
                      ...
                      {lesson.sections[activeSection].codeExample && (
                        <div className="space-y-2">
                          <div className="flex items-center justify-between">
                            <span className="text-[10px] text-slate-500 uppercase tracking-widest font-bold">Code Example</span>
                            <button
                          ...
                          <pre className="bg-slate-950 border border-slate-800 rounded-xl p-4 text-sm font-mono text-emerald-400 overflow-x-auto whitespace-pre-wrap">
                            {lesson.sections[activeSection].codeExample}
                          </pre>
                        </div>
                      )}
                      ...
                      {lesson.sections[activeSection].exercise && (
                        <div className="bg-violet-600/5 border border-violet-500/20 rounded-2xl p-5 space-y-3">
                          <div className="flex items-center gap-2">
                            <span className="text-violet-400 text-lg">🏋️</span>
                            <span className="text-sm font-bold text-violet-400 uppercase tracking-wider">Exercise</span>
                          </div>
                          ...
                          {lesson.sections[activeSection].exercise.hint && (
                            <details className="text-xs text-slate-500">
                              <summary className="cursor-pointer hover:text-violet-400 font-bold">💡 Show Hint</summary>
                              <p className="mt-2 text-slate-400 pl-4 border-l-2 border-violet-500/30">{lesson.sections[activeSection].exercise.hint}</p>
                            </details>
                          )}
                          <button
                            onClick={() => {
                            ...
                            className="text-xs bg-violet-600 hover:bg-violet-500 text-white px-4 py-2 rounded-lg font-bold transition-colors"
                          >
                      ...
                      <div className="flex items-center justify-between pt-4 border-t border-slate-800">
                        <button
                          ...
                          className="text-xs text-slate-500 hover:text-white disabled:opacity-30 font-bold"
                        >
                          ← Previous
                        </button>
                        <span className="text-[10px] text-slate-600">{activeSection + 1} / {lesson.sections.length}</span>
                        <button
                          ...
                          className="text-xs text-violet-400 hover:text-violet-300 disabled:opacity-30 font-bold"
                        >
                          Next →
                        </button>
                      </div>
          ...
          {leftTab === "chat" && (
            <div className="flex-1 flex flex-col overflow-hidden">
              <div className="flex-1 overflow-y-auto p-4 space-y-4">
                ...
                {chatMessages.map((msg, idx) => (
                  <div key={idx} className={`flex flex-col ${msg.role === "user" ? "items-end" : "items-start"}`}>
                    <span className="text-[9px] text-slate-600 mb-1 font-bold uppercase tracking-wider">
              ...
              <form onSubmit={handleChatSubmit} className="p-3 border-t border-slate-800 bg-slate-900 flex items-center gap-2 shrink-0">
                <input
                  type="text"
                  value={chatInput}
                  ...
                  className="flex-1 bg-slate-950 border border-slate-800 rounded-xl px-4 py-2.5 text-sm text-slate-200 outline-none focus:border-violet-500 disabled:opacity-50 placeholder:text-slate-600"
                />
                <button
                  type="submit"
                  disabled={!chatInput.trim() || loadingChat || !currentTopic}
                  className="bg-violet-600 text-white px-4 py-2.5 rounded-xl text-sm font-bold disabled:opacity-50 transition-all"
                >
                  Send
                </button>
              </form>
        ...
        <div className="flex-1 flex flex-col bg-[#1e1e1e]">
          ...
          <div className="h-12 bg-slate-900 border-b border-slate-800 flex items-center justify-between px-4 shrink-0">
            <div className="flex items-center gap-3">
              <select
                ...
                className="bg-slate-950 border border-slate-800 text-slate-200 text-xs rounded-lg px-3 py-1.5 outline-none disabled:opacity-50 disabled:cursor-not-allowed"
              >
            ...
            <button
              ...
              className="bg-emerald-500 hover:bg-emerald-400 text-white text-xs font-bold px-5 py-2 rounded-lg disabled:opacity-50 transition-all flex items-center gap-2 shadow-lg shadow-emerald-500/20"
            >
              {isExecuting ? (
                <>
          ...
          {/* Output Panel */}
          <div className="h-[200px] min-h-[150px] bg-slate-950 border-t border-slate-800 flex flex-col shrink-0">
            <div className="text-[10px] font-bold text-slate-500 uppercase tracking-wider p-3 border-b border-slate-800 bg-slate-900 flex items-center justify-between">
              <span>Output</span>
              {executionOutput && (
                <span className={`text-[9px] px-2 py-0.5 rounded-full ${executionOutput.statusCode === 200 ? "bg-emerald-500/10 text-emerald-400 border border-emerald-500/20" : "bg-rose-500/10 text-rose-400 border border-rose-500/20"}`}>
            ...
            <div className="flex-1 p-0 overflow-hidden relative">
              {frontendHtml ? (
                ...
                <div className="p-3 h-full overflow-y-auto">
                  <pre className={`text-sm font-mono whitespace-pre-wrap ${executionOutput.statusCode !== 200 ? "text-rose-400" : "text-slate-300"}`}>
                    {executionOutput.output || "No output"}
                  </pre>
                </div>
              ) : (
                <div className="p-3 text-slate-600 text-xs italic">Run your code to see output here...</div>
              )}
            </div>
          </div>
        </div>
